...
def plot_coverage(gene, name, trk_border):
    ...
    if ttest_res is not None:
        title += f'p = {round(ttest_res, 3)} (U test)\n'

    title += f'non-TK/TK coverage = {round(cov_non_trk_raw,2)}/{round(cov_trk_raw,2)}'
    ...
    y_min = -max(cov_down) * 1.1 if max(cov_down) > 0 else -0.05
    plt.ylim(y_min, 0.7)
...
if __name__ == '__main__':
    ...
    genes_dict = exons_from_table(f'{script_path}/all_tkr_exons.tsv', gene_list, no_intron=True)
    ...
    for gene in gene_list:
        ...
        if len(trk_exons) > 0:
            ...
            ttest_res = scp.mannwhitneyu(cov_non_trk, cov_trk, alternative='less').pvalue
        else:
            ttest_res = None
